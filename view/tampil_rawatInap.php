...
<html lang="en">
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
	...
	<div class="card text-center" style="width: 97%;">
	  <div class="card-header">
	    <ul class="nav nav-tabs card-header-tabs">
	      ...
	      <li class="nav-link">
	      	<a class="nav-link active" href="tampil_rawatInap.php">Daftar Rawat Inap</a>
	      </li>
	    </ul>
	  </div>
	  <div class="card-body text-left">
			<table style="width: 100%;">
				<tr>
					<td>	
					<button class="btn btn-success" style="float: right;right: 0;" type="button" data-toggle="modal" data-target="#tambahRawatInap">Rawat Inap</button>
					</td>
				</tr>
			</table>
		    <?php 
				include 'tabel_daftar_rawatInap.php';
			 ?>
		<!-- akhir komom dan row -->
	  </div>
	</div>
	...
	<!-- modal tambah -->
	<!-- Modal -->
	<div class="modal fade" id="tambahRawatInap" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
	  <div class="modal-dialog modal-dialog-scrollable" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h5 class="modal-title">Tambah Rawat Inap</h5>
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	          <span aria-hidden="true">&times;</span>
	        </button>
	      </div>
	      <div class="modal-body">
	       <h3>Registrasi Rawat Inap</h3>
	       <form action="../proses/proses_simpan_rawatInap.php" method="post" enctype="multipart/form-data">
	       	<?php 
	       		$query_pasien = mysqli_query($koneksi, "select * from daftar_pasien");
	       		$query_dokter = mysqli_query($koneksi, "select * from daftar_dokter");
	       		$query_ruangan = mysqli_query($koneksi, "select * from daftar_ruangan");
	       			?>
	       		...
	       		<div class="form-group">
	       			...
	       			<select class="form-control" name="pasien">
	       				...
	       				<?php 
	       				 ...
	       				 ?>
	       				<option value="<?php echo $data_pasien['id'] ?>"><?php echo $data_pasien['id'] ?> | <?php echo $data_pasien['nama'] ?></option>
						<?php
				       		}
				       	 ?>
	       			</select>
	       			...
	       			<select name="dokter" class="form-control">
	       				...
	       				<?php 
	       					while ($data_dokter =  mysqli_fetch_array($query_dokter)) {
	       						?>
	       						<option value="<?php echo $data_dokter['id'] ?>"><?php echo $data_dokter['id'] ?> | <?php echo $data_dokter['nama'] ?></option>
	       						<?php
	       					}
	       				 ?>
	       		</select>
	       			...
	       			<select name="ruangan" class="form-control">
	       				...
	       				<?php 
	       					while($data_ruangan = mysqli_fetch_array($query_ruangan)){
	       						?>
	       						<option value="<?php echo $data_ruangan['id'] ?>"><?php echo $data_ruangan['id'] ?> | <?php echo $data_ruangan['nama_ruangan'] ?></option>
	       						<?php
	       						}
	       				 ?>
	       			</select>
